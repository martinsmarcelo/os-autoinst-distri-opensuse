...
sub check_failures_in_journal {
    ...
    my @warnings = ('Started Process Core Dump', 'sysfs group', 'Call Trace');
    foreach (@warnings) {
        $cmd = "grep '$_' $logfile";
        $cmd = "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root\@$machine " . "\"$cmd\"" if $machine ne 'localhost';
	...
	#bsc1205967 know bug  
	elsif ($failures eq "sysfs group") {
            record_soft_failure("BSC#1205967 \n");
            record_info("Found know bug BSC#1205967 failures in journal log. \n", result => 'fail');
        }
        else {
